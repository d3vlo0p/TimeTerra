...
2.  **Reconciliation Layer (`internal/controller`)**:
    -   **Generic Pattern**: Most target controllers embed `BaseReconciler` and use the `reconcileResource` function in `resource_controller.go`.
...
3.  **Action Layer (Target Controllers & Async Handlers)**:
    ...
    -   For complex, asynchronous operations (like DB vertical scaling on AWS), controllers instantiate an `ActionExecution` CR, which is reconciled independently by `ActionExecutionReconciler` and executes specific action handlers (e.g., `AuroraScaleHandler` or `DocumentDbScaleHandler`).

## Action Execution Workflow

For asynchronous operations (like vertical scaling):
1.  **Creation**: The target controller (e.g., `AwsRdsAuroraClusterReconciler`) creates an `ActionExecution` custom resource.
2.  **Execution**: `ActionExecutionReconciler` catches the new resource, executes the matching handler (`internal/action/`), and polls until completion.
3.  **Finalization**: Once done (Success/Error), the controller sends notifications, updates status conditions and events on the parent resource, and deletes the `ActionExecution` resource.

## Key Interfaces & Types

Found in `internal/controller/types.go` and `resource_controller.go`:

-   **`BaseReconciler`**: Struct holding common manager dependencies (`Client`, `Scheme`, `Cron` schedule service, `NotificationService`, and `EventRecorder`) that is embedded by individual resource reconcilers.
...
3.  **Implement Logic**:
    -   Follow the pattern in `internal/controller/awsec2instance_controller.go` (ensure the reconciler embeds `BaseReconciler`).
